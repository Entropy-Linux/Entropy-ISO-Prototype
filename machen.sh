...
clear && echo "STATUS:" | boxes -d warning
pwd && export ARCHLIVE=$(pwd) 
echo "Path" $ARCHLIVE | boxes -d warning
ls /var/tmp | grep 'Entropy'
sleep 1
ls /var/tmp/out | grep '.iso'
sleep 1
df /var/tmp 
sleep 4
...
choices=(
"Build ISO" ".build-scripts/build.sh"
"Build FS" ".build-scripts/buildfs.sh"
"Build Repo" ".build-scripts/build-repo.sh"
"Test ISO" ".build-scripts/test.sh"
"Clean Work" ".build-scripts/clean.sh"
)
